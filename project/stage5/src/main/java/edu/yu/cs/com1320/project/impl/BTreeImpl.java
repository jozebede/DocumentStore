...
public class BTreeImpl<Key extends Comparable<Key>, Value> implements BTree {
    ...
    public Object get(Comparable k) {
            ...
            if ( entry.val != null) {
                return (Object) entry.val;
            }

            try {
                Object doc = pm.deserialize((URI)k);
                put(k,doc);
                return doc;
    ...
    public Object put(Comparable k, Object v) {
        // Should return what used to be at the given key (null if nothing was there)
            if (k == null)
            {
                throw new IllegalArgumentException("argument key to put() is null");
                // tengo q hacer return?
            }
            ...
            {
                Entry  result = alreadyThere;
                alreadyThere.val = v;
                return result;
            }

            // if k doesnt exist, create new node
            Node newNode = this.put(this.root, k, v, this.height);
            this.n++;
            if (newNode == null)
            {
                // if no new node was created
                return v ; //  no seguro
            }
         ...
         */
        private Node put(Node currentNode, Comparable key, Object val, int height) {
            int j;
    ...
    public void moveToDisk(Comparable k) throws Exception {
       ...
       pm.serialize((URI) k, (Document) get(k));
       put (k,null);
